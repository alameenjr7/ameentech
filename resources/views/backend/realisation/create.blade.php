...
<div class="ui-kit-cards grid mb-24">
    {{-- <h3>Complex</h3> --}}
    <form class="row g-3" class="row g-3" action="{{route('realisation.store')}}" method="POST">
        @csrf
        <div class="col-md-6">
        ...
        <div class="col-md-6">
            ...
            <select name="client_id" id="client_id" class="form-control form-control-rounded">
                ...
                @foreach (\App\Models\Client::where('status','active')->get() as $client)
                    <option value="{{$client->id}}">{{ucfirst($client->prenom)}} {{ucfirst($client->nom)}}</option>
                @endforeach
            </select>
        </div>
        {{-- Langage --}}
        ...
        <div class="col-md-6">
            ...
            <select id="inputState" name="condition" class="form-select form-control">
                ...
                <option value="terminer" {{old('condition')=='terminer' ? 'selected' : ''}}>Terminer</option>
                <option value="en_cours" {{old('condition')=='en_cours' ? 'selected' : ''}}>En Cours</option>
                <option value="future" {{old('condition')=='future' ? 'selected' : ''}}>Future</option>
            </select>
        </div>
        <div class="col-md-6">
        ...
        <div class="col-12">
            ...
            <textarea type="text" class="form-control" id="description" placeholder="1234 Main St" name="contenu">{{old('contenu')}}</textarea>
        </div>
        <div class="col-12">
